...
The 8 unit types, what they do, and the rules of engagement. Deterministic
...
**Rule that replaces "sigint":** any unit with radar **ON** is visible to all
enemies at long range (the emission gives them away). Turning radar OFF makes
you stealthier but you only see your passive radius.
...
| HOLD              | Skip the turn. |
| CAPTURE           | Ground unit on an objective hex; ticks the hold counter. |
...
- **Move-out dodge**: strikes resolve **after** the MOVE phase. If the target
  hex is empty by then, the strike whiffs.
- **Ammo**: `∞` never decrements; finite ammo decrements per shot. `0` ammo
  disables STRIKE.
...
1. **Objective hold** — held all objective hexes uncontested for 3 consecutive
   turns → win.
2. **Annihilation** — opposing side has 0 units AND 0 bases → win.
3. **HP collapse** — total HP (units + bases) ≤ 25% of starting → other side
   wins.
4. **Turn cap** — turn ≥ 30 → higher HP% wins; tie = draw.
...
|----|------|------|-------|---------|--------------------------------------|
| 4  | 2    | 2    | —     | -       | MOVE, STRIKE, OVERWATCH, HOLD, CAPTURE |

| Weapon | Range | Damage | Ammo | Hits         |
|--------|-------|--------|------|--------------|
| rifle / MANPADS | 1 | 1 | ∞ | ground, air  |

Only ground unit that can capture.
...
| HP | Move | View | Radar     | Stealth | Actions                                                      |
|----|------|------|-----------|---------|--------------------------------------------------------------|
| 3  | 0    | 1    | range 4   | -       | STRIKE, OVERWATCH, ACTIVATE_RADAR, DEACTIVATE_RADAR, HOLD    |
...
- **MOVE**: fly up to 4 hexes, sees enemies in radius 2 around its new position (passive view).
...
|--------|-------|--------|------|-----------|----------------------|
| OWA warhead | 0 | 3 | 1 | ground, sea, air | self-destruct: attacker dies after firing |
...
| HP | Move | View | Radar    | Stealth | Actions                                                          |
|----|------|------|----------|---------|------------------------------------------------------------------|
| 3  | 4    | 1    | range 3  | Y       | MOVE, STRIKE, OVERWATCH, ACTIVATE_RADAR, DEACTIVATE_RADAR, HOLD  |

| Weapon | Range | Damage | Ammo | Hits             |
|--------|-------|--------|------|------------------|
| AAM/ASM | 3    | 2      | 4    | ground, air, sea |

Stealth halves enemy detector ranges against it. Radar ON gives it 3-hex
sight but breaks the stealth advantage at long range. No SCOUT action —
that's drone work.
...
| HP | Move | View | Radar    | Stealth | Actions                                                       |
|----|------|------|----------|---------|---------------------------------------------------------------|
| 8  | 2    | 1    | range 3  | -       | MOVE, STRIKE, OVERWATCH, ACTIVATE_RADAR, DEACTIVATE_RADAR, HOLD |
...
| HP | Move | View | Radar    | Stealth | Actions                                       |
|----|------|------|----------|---------|-----------------------------------------------|
| 12 | 0    | 2    | range 4  | -       | ACTIVATE_RADAR, DEACTIVATE_RADAR, HOLD        |
...
| Type             | Dom | HP | Mv | View | Radar | Wpn rng | Wpn dmg | Ammo | Stealth | Capture |
|------------------|-----|----|----|------|-------|---------|---------|------|---------|---------|
...
Need a yes/no before catalog rewrite:
...
4. **Stealth halves view range only, or radar too?** (My take: both — sigint is gone, so this is the only stealth mechanic.)
5. **Last-known-position memory** — does it persist forever (until the unit moves) or decay after N turns even if stationary? (My take: forever; it only takes one move to clear it.)
