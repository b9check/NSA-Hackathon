...
# Sensor ranges are intentionally brutal: default 0 (own hex only), so the
# game forces *active scouting*. Only dedicated airborne ISR platforms see
# beyond their own hex. Combined-arms doctrine emerges: SAMs and fighters
# have long weapon range but no eyes — they need a UAV / forward unit to
# cue them. Kill the enemy's UAV and you've blinded them.
CATALOG: dict[str, UnitType] = {
    # ---------------- Blue ----------------
    "f35": UnitType(
        key="f35", display="F-35", side="blue", domain="air", glyph="A",
        speed=6, sensor=0, weapon=4, hp=3, cost=100, stealth=True,
    ),
    "mq9": UnitType(
        key="mq9", display="MQ-9 Reaper", side="blue", domain="air", glyph="U",
        speed=4, sensor=1, weapon=3, hp=1, cost=30,
    ),
    "aegis": UnitType(
        key="aegis", display="Aegis DDG", side="blue", domain="sea", glyph="N",
        speed=2, sensor=0, weapon=5, hp=8, cost=120,
    ),
    "patriot": UnitType(
        key="patriot", display="Patriot SAM", side="blue", domain="land", glyph="S",
        speed=1, sensor=0, weapon=5, hp=3, cost=60,
    ),
    "m1a2": UnitType(
        key="m1a2", display="M1A2 Abrams", side="blue", domain="land", glyph="T",
        speed=2, sensor=0, weapon=2, hp=6, cost=50,
    ),
    "mech_b": UnitType(
        key="mech_b", display="Mech Infantry", side="blue", domain="land", glyph="I",
        speed=2, sensor=0, weapon=2, hp=4, cost=30,
    ),
    # ---------------- Red ----------------
    "j20": UnitType(
